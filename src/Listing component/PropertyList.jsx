...
export const PropertyList = () => {
  ...
  const handleInterestToggle = (propertyId, propertyName) => {
    if (!interestStatus[propertyId]) {
      ...
      alert(`You are already interested in ${propertyName}`);
    }
  };
      ...
      {properties.map(property => (
        <div className="property" key={property.id}>
          ...
          <div className="property-details">
            ...
            <button onClick={() => handleInterestToggle(property.id, property.name)}>
              {interestStatus[property.id] ? 'Interested' : 'Show Interest'}
            </button>
          </div>
        </div>
      ))}
